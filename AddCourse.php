    ...
    <div class="container">
        <div class="row mt-4">
            ...
            <div class="col-sm-6 p-4 my-5 bg-primary border-radius-2 rounded">
                <h2 class="p-2 mb-4 text-center text-white">Add Course</h2>
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                    <div class="form-group my-3">
                        <label class="text-white">Course Code:</label>
                        <input type="text" class="form-control" name="course_code" placeholder="Enter course code" value="<?php if (isset($course_code) && $courseCheck != false) { echo $course_code; } ?>">
                        <span class="error text-danger"><?php echo $codeErr; ?></span>
                    </div>
                    <div class="form-group my-3">
                        <label class="text-white">Course Name:</label>
                        <input type="text" class="form-control" name="course_name" placeholder="Enter course name" value="<?php if (isset($course_name) && $courseCheck != false) {echo $course_name;} ?>">
                        <span class="error text-danger"><?php echo $courseNameErr; ?></span>
                    </div>
                    <div class="form-group row my-3">
                        <div class="col-sm-6">
                            <label class="text-white">Maximum Number of Enrollees:</label>
                            <input type="number" class="form-control" name="max" placeholder="Set the maximum capacity" value="<?php if (isset($max) && $courseCheck != false) {echo $max;} ?>">
                            <span class="error text-danger"><?php echo $maxErr; ?></span>
                        </div>
                        <div class="col-sm-6">
                            <label class="text-white">Number of Units:</label>
                            <input type="number" class="form-control" name="units" placeholder="Set the number of units" value="<?php if (isset($units) && $courseCheck != false) {echo $units;} ?>">
                    ...
                    <div class="md-form">
                        <div class="d-flex bd-highlight justify-content-center mt-5">
                            <div class="bd-highlight">
                                <input type="submit" value="Submit" class="btn btn-success btn-lg text-white" name="add_course">
                            </div>
                        </div>
                    </div>
                    <div class="text-center mt-3">
                        <span class="error text-danger text-center"><?php echo $courseErr; ?></span>
                    </div>
                </form>
            </div>
        </div>
